...
export default function Cursos({items ,id,nivel, idCurso }:{id:string, items:any, nivel:string, idCurso:string}){
      ...
      const moveItemOpem = ()=>{
       setOpne(true)
           ...
           >
             ...
             <Link href={`/cursos/sobre/${item._id}`} className="lg:w-[12rem] md:w-[10rem] max-sm:w-[7rem] cursor-pointer h-[2rem]  justify-center items-center bg-[#6BA614] flex rounded-2xl mt-[24px]">
                <span className="text-white lg:text-[12px] md:text-[10px] max-sm:text-[8px] font-semibold">Saber mais</span>
             ...
             {open &&(
                  ...
                  item.disciplina.map((items:any, key:any)=>(
                    ...
                    <Link href={/**perfil/aulas/${id?id:"1"}/${nivel?nivel:"1"}/${item._id}` */`#`}>
