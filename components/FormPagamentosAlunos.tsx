'use client'
import { UpdatePropinaPedido } from "@/lib/actions/alunos.actions";
...
export default function FormPagamentosAlunos(
    {id, 
    data, 
    nome,
    imagem,
        ...
        <form  onSubmit={(e)=>handelSubmit(e)} data-aos='fade-down' action=""  className="flex flex-col  text-[#000] py-7 w-full  justify-center items-center ">
                   <div>
                          ...
                          >
                            {({ open }) => {
                              return (
                                <button className="px-4 py-2 bg-green-600" onClick={() => open()}>
                                  Upload an Image
                                </button>
                              );
                            }}
                   ...
                   <div>
                    ...
                    <CldImage
                      ...
                      className="m-4 rounded-lg border-[2px] border-green-400"
                    />
 
 
                   </div>
                     <select className="p-2 mb-4 rounded-3xl hover:bg-green-800 w-[100%] bg-transparent border border-green-600 text-white bg-black max-sm:text-[12px] text-[18px] font-bold" name="modeloPagamento" onChange={(e)=>handelChange(e)} id="">
               ...
               {dados?.modeloPagamento? "" : <span className="text-center text-red-600 text-[18px] font-bold">Porfavor selecione primeiro a quantidade de meses a se pagar ! </span> }
                    <div className={dados?.modeloPagamento?'w-[100%] grid  lg:grid-cols-3 md:grid-cols-3 max-sm:grid-cols-2  text-white p-[2rem] gap-[2rem]' : 'w-full grid  lg:grid-cols-3 md:grid-cols-3 max-sm:grid-cols-2  text-white p-[0.5rem] gap-2 disabled: opacity-75 disabled:pointer-events-none' } >
                         ...
                         {meses.map((item, key)=>(
                           <div key={key} className={`flex lg:text-[12px] md:text-[12px] max-sm:text-[12px] font-bold border-[4px] rounded-2xl items-center hover:scale-110 transition-all duration-300  justify-center p-[0.5rem] mx-[6px]   ${item.sec <= mesesPagos[0].meses.length ? 'border-green-600' : 'border-red-600'} `}>
